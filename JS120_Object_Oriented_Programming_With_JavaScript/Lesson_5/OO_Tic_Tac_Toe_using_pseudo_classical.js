...
Board.prototype.countMarkersFor = function(player, keys) {
  let marker = keys.filter(key => this.squares[key].getMarker() === player.getMarker());
...
TTTGame.prototype.isWinner = function(player) {
  return TTTGame.POSSIBLE_WINNING_ROWS.some(row => this.board.countMarkersFor(player, row) === 3);
}
